...
const DPPs: React.FC<DPPsProps> = ({ dpps }) => {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(false);
    ...
    const [showSolution, setShowSolution] = useState<{ [questionId: string]: boolean }>({});
            ...
            <View>
                {showDppList ? (
                  ...
                  <View className="px-3">
                      ...
                      {currentDpp && currentDpp.dppQuestions.map((question, index) => {
                        ...
                        return (
                          <View key={question._id} className="mb-4 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                            ...
                            {/* Show solution image if present */}
                            {question.solutionImage ? (
                              <View className="mt-3">
                                <TouchableOpacity
                                  onPress={() => handleToggleSolution(question._id)}
                                  className="bg-blue-100 px-4 py-2 rounded-lg mb-2"
                                >
                                  <Text className="text-blue-700 font-medium">
                                    {showSolution[question._id] ? 'Hide Solution' : 'Show Solution'}
                                  </Text>
                                </TouchableOpacity>
                    ...
                    {/* Full Screen Modal for Solution Image */}
                    <Modal
                    ...
                    >
                      ...
                      >
                        ...
                        <Text style={{ color: '#fff', marginTop: 16, fontSize: 16 }}>Tap anywhere to close</Text>
                      </Pressable>
                    </Modal>
